...
export default function Home() {
  ...
  async function confirmAndAnalyze() {
    ...
    const domain = window.prompt("Domain (optional)") ?? "";
  ...
  async function decide(item: Suggestion, action: "ACCEPT"|"EDIT_AND_ACCEPT"|"REJECT") {
    const edited = action === "EDIT_AND_ACCEPT" ? window.prompt("Edit the proposed text", item.after) : null;
    if (action === "EDIT_AND_ACCEPT" && !edited) return;
    const reason = action === "REJECT" ? window.prompt("Why are you rejecting this suggestion?") : null;
  ...
  return <main>
    ...
    <form onSubmit={importManual} className="editor">
      ...
      {importId && <button className="secondary" type="button" onClick={confirmAndAnalyze}>Confirm facts & run audit</button>}
