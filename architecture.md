...
The retrieval query first enforces workspace, eligibility, taxonomy, exact approval, latest-revision, and state constraints in SQL. `v0.2D` stores 64-dimensional `hash-embedding@1` vectors in PostgreSQL pgvector (JSON through the SQLite compatibility adapter), ranks the small personal corpus deterministically in application code, and records selected revision IDs, taxonomy/embedding versions, and similarity scores. This establishes a replaceable embedding contract without sending private memory to another provider; pgvector-native nearest-neighbor queries and indexes become an optimization when corpus size warrants them. Published revisions remain eligible once the publishing lifecycle exists; currently, approved unpublished revisions require current exact approval. Rejected, deleted, disabled, and superseded-unpublished revisions are excluded.
